...
def plot_villes(villes, chemin):
    x = [villes[i][0] for i in chemin] + [villes[chemin[0]][0]]
    y = [villes[i][1] for i in chemin] + [villes[chemin[0]][1]]

    fig, ax = plt.subplots()
    ax.plot(x, y, '-o', color='blue')
    for i, (xv, yv) in enumerate(villes):
        ax.text(xv, yv + 0.5, f"Ville {i}", fontsize=9, color="red")
    ax.set_title("Visualisation du chemin optimal")
    return fig
